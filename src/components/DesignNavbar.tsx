...
const DesignNavbar = ({ lang, theme, userName, activeTab, onTabChange, onToggleLang, onToggleTheme, onSwitchApp }: DesignNavbarProps) => {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-surface-sombre/90 backdrop-blur-[40px] saturate-[180%] border-b border-border/30">
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <span className="font-clash font-bold text-xl text-or">MG<span className="text-xs ml-1 text-muted-foreground">Design</span></span>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onToggleLang} className="px-2 py-1 rounded-lg text-xs font-clash font-bold uppercase text-muted-foreground hover:text-foreground transition-colors">
            {lang === 'fr' ? 'EN' : 'FR'}
          </button>
          <button onClick={onToggleTheme} className="p-2 rounded-lg text-muted-foreground hover:text-foreground transition-colors">
            {theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}
          </button>
          <div className="ml-2 px-3 py-1 rounded-lg bg-or/15 text-or font-satoshi text-sm font-medium">
            {userName}
          </div>
        </div>
      </div>
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg font-satoshi text-xs font-medium whitespace-nowrap transition-all ${
              activeTab === tab ? 'bg-or/15 text-or' : 'text-foreground/70 hover:text-foreground'
            }`}
          >
            {t(tabLabelKeys[tab], lang)}
          </button>
